...
<div class="col-xs-12">
    <form id="table-bar-form">
        <div class="table-bar form-inline">
            <a href="/admin/blog/add" class="btn btn-small btn-warning">
...
<script src="/wpadmin/lib/vison/store.js"></script>
<script>
                $(function () {
                    ...
                    $.zui.store.pageClear();
                    $("#list").jqGrid({
                        url: "/admin/blog/getDataList",
                        datatype: "json",
                        mtype: "POST",
                        colNames:
                                ['类别', '用户', '标题', 'seo关键字', 'seo描述', '创建时间', '更新时间', '点击量', '操作'],
                        colModel: [
                            {name: 'blogcat.name', editable: true, align: 'center'},
                            {name: 'admin.nick', editable: true, align: 'center'},
                            {name: 'title', editable: true, align: 'center'},
                            {name: 'keywords', editable: true, align: 'center'},
                            {name: 'description', editable: true, align: 'center'},
                            {name: 'ctime', editable: true, align: 'center'},
                            {name: 'updatetime', editable: true, align: 'center'},
                ...
                function actionFormatter(cellvalue, options, rowObject) {
                    ...
                    response += '<a title="编辑" href="/admin/blog/edit/' + rowObject.id + '" class="grid-btn "><i class="icon icon-pencil"></i> </a>';
                    return response;
                }
                ...
                function doExport() {
                    ...
                    var searchData = Store.pageGet('searchData') ? Store.pageGet('searchData') : {};
                    searchData['sidx'] = sortColumnName;
                    searchData['sort'] = sortOrder;
                    var searchQueryStr = $.param(searchData);
                    $("body").append("<iframe src='/admin/blog/exportExcel?" + searchQueryStr + "' style='display: none;' ></iframe>");
                }

                function doView(id) {
                    //查看明细
                    url = '/admin/blog/view/' + id;
                    layer.open({
                        type: 2,
                        title: '查看详情',
                        shadeClose: true,
